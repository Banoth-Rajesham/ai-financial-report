...
def generate_swot_analysis(kpis):
    ...
    strengths_html = "".join(strengths) if strengths else "<li>Key strengths not immediately apparent from ratios.</li>"
    weaknesses_html = "".join(weaknesses) if weaknesses else "<li>Financial ratios indicate a generally stable position.</li>"
    swot_md = f"""
    <div class="chart-container" style="padding: 1.5rem; color: #e0e0e0;">
    <h4>SWOT Analysis</h4>
...
# ... (Session state init is correct and unchanged) ...
...
if not st.session_state.report_generated:
    ...
    # ... (KPI calculations are correct and unchanged) ...
    st.markdown(f"""<div class="kpi-container">...</div>""", unsafe_allow_html=True) # Your KPI cards
    ...
    # --- THIS IS THE ONLY SECTION THAT IS MODIFIED ---
    st.write("---")
