...
# This version correctly handles a single, potentially modified Excel file
# from the first part of the application to generate a PDF report.
# ==============================================================================
...
if st.button("Generate PDF Report", type="secondary", use_container_width=True, key="generate_pdf_button"):
    if excel_file and company_name_pdf:
        with st.spinner("Processing Excel file and generating report..."):
            try:
                # Read the specific sheet generated by the app
                df = pd.read_excel(excel_file, sheet_name='Aggregated Statement', skiprows=2)
                
                # Check for the correct columns
                if 'Particulars' in df.columns and 'CY' in df.columns and 'PY' in df.columns:
                    # Create a dictionary to hold our aggregated data based on keywords
                    agg_data_from_excel = {}

                    def find_value(keyword):
                        row = df[df['Particulars'].str.contains(keyword, na=False)]
                        if not row.empty:
                    ...
                    def find_sub_item_value(main_keyword, sub_keyword):
                        ...
                        main_row_index = df[df['Particulars'].str.contains(main_keyword, na=False)].index
                        if not main_row_index.empty:
                            # Search for the sub_keyword immediately after the main row
                            for i in range(main_row_index[0] + 1, len(df)):
                                if sub_keyword in str(df.iloc[i]['Particulars']):
                                    return df.iloc[i]['CY'], df.iloc[i]['PY']
                        return 0, 0

                    # Now, map values from the DataFrame to the required aggregated_data structure
                    cy_equity, py_equity = find_value("Total Equity")
                    ...
                    cy_profit, py_profit = find_value("Net Profit")
                    
                    # Construct the aggregated data dictionary
                    agg_data_from_excel = {
                        '1': {'total': {'CY': cy_equity, 'PY': py_equity}},
                        '2': {'total': {'CY': 0, 'PY': 0}}, # This is a placeholder as `Total Equity` is a single line
                        '3': {'total': {'CY': cy_debt, 'PY': py_debt}},
                        '7': {'total': {'CY': 0, 'PY': 0}}, # This is a placeholder as `Total Debt` is a single line
                        '11': {'total': {'CY': cy_assets, 'PY': py_assets}},
                        '16': {'total': {'CY': 0, 'PY': 0}}, # Placeholder as this is not in the summary
                        '21': {'total': {'CY': cy_revenue, 'PY': py_revenue}},
                        '22': {'total': {'CY': 0, 'PY': 0}}, # Placeholder as total revenue is one line
                    ...
                    # We need to manually calculate the depreciation to get an accurate profit
                    # Let's try to get it from the a specific note sheet if available.
                    # This is an assumption based on the original structure.
                    depreciation_cy, depreciation_py = find_sub_item_value("Tangible assets", "Depreciation")
                    ...
                    st.error("The uploaded Excel file's 'Aggregated Statement' sheet does not have the expected columns: 'Particulars', 'CY', 'PY'.")
            except Exception as e:
                st.error(f"An error occurred while processing the Excel file. Please ensure you have uploaded a single Excel file generated by this application. Error: {e}")
    else:
