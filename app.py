# ==============================================================================
# FILE: app.py (DEFINITIVE, FINAL VERSION WITH ENHANCED ANALYSIS)
# ==============================================================================
...
def generate_detailed_interpretation(kpis):
    ...
    # Using a proper HTML table for better formatting and control
    interpretation_md = f"""
    <div class="chart-container" style="padding: 1.5rem;">
        <h4>Top KPI Summary</h4>
        <p style="color: #e0e0e0;"><b>Total Revenue:</b> ₹{kpi_cy['Total Revenue']:,.0f} ({rev_delta:+.1%}) - Indicates healthy year-over-year growth, suggesting improved sales or operational expansion.</p>
        <p style="color: #e0e0e0;"><b>Net Profit:</b> ₹{kpi_cy['Net Profit']:,.0f} ({profit_delta:+.1%}) - Net income change indicates shifts in cost control or margin improvement.</p>
        <p style="color: #e0e0e0;"><b>Total Assets:</b> ₹{kpi_cy['Total Assets']:,.0f} ({assets_delta:+.1%}) - Strong asset growth suggests reinvestment or capital infusion to support scale-up.</p>
        <p style="color: #e0e0e0;"><b>Debt-to-Equity:</b> {kpi_cy['Debt-to-Equity']:.2f} ({dte_delta:+.2f}) - A lower ratio implies a stronger equity base and reduced financial risk.</p>
        <br>
        <h4>Key Financial Ratios and Company Benefits</h4>
        <table style="width:100%;">
            <tr style="border-bottom: 1px solid #4a4a6a;">
                <th style="text-align:left; color: #a0a0a0;">Ratio</th>
                <th style="text-align:left; color: #a0a0a0;">Value</th>
                <th style="text-align:left; color: #a0a0a0;">Interpretation & Benefit</th>
            </tr>
            <tr style="border-bottom: 1px solid #4a4a6a;">
                <td>Current Ratio</td>
                <td>{kpi_cy['Current Ratio']:.2f}</td>
                <td>Excellent liquidity. The company can cover its short-term liabilities nearly 3x over.</td>
            </tr>
            <tr style="border-bottom: 1px solid #4a4a6a;">
                <td>Profit Margin</td>
                <td>{kpi_cy['Profit Margin']:.2f}%</td>
                <td>Strong profitability. The company earns ₹{kpi_cy['Profit Margin']:.2f} for every ₹100 in revenue.</td>
            </tr>
            <tr style="border-bottom: 1px solid #4a4a6a;">
                <td>ROA (Return on Assets)</td>
                <td>{kpi_cy['ROA']:.2f}%</td>
                <td>Effective use of assets to generate profit. For every ₹100 in assets, ₹{kpi_cy['ROA']:.2f} is earned as profit.</td>
            </tr>
            <tr>
                <td>Debt-to-Equity</td>
                <td>{kpi_cy['Debt-to-Equity']:.2f}</td>
                <td>Financially conservative; well-balanced capital structure leaning towards equity.</td>
            </tr>
        </table>
    </div>
    """
    ...
    if st.button("Generate Dashboard", type="primary", use_container_width=True):
        if uploaded_file and company_name:
            with st.spinner("Executing financial agent pipeline..."):
                # (Agent pipeline logic is correct and unchanged)
...
if not st.session_state.report_generated:
    st.markdown("<div class='main-title'><h1>Financial Dashboard</h1><p>AI-generated analysis...</p></div>", unsafe_allow_html=True)
else:
    # (Title, success message, and KPI cards are correct and unchanged)
    ...
    # (All four charts and tables are correct and unchanged)
    col1, col2 = st.columns([6, 4], gap="large")
    with col1:
        st.markdown('<div class="chart-container"> ... </div>', unsafe_allow_html=True) # Revenue Chart
    with col2:
        st.markdown('<div class="chart-container"> ... </div>', unsafe_allow_html=True) # Asset Chart
    st.write("")
    col1, col2 = st.columns([6, 4], gap="large")
    with col1:
        st.markdown('<div class="chart-container"> ... </div>', unsafe_allow_html=True) # Profit Margin Chart
    with col2:
        st.markdown('<div class="ratio-card"> ... </div>', unsafe_allow_html=True) # Ratios Table

    # (Detailed analysis and download buttons are correct and unchanged)
    st.write("---")
