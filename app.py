...
try:
    from config import NOTES_STRUCTURE_AND_MAPPING, SCHEDULE_III_CONFIG
    from agents import (
...
if st.session_state.report_generated:
    ...
    # ========================================================== #
    # == THIS IS THE FIX: The download buttons are now INSIDE == #
    # == this "if" block, so they only try to exist when      == #
    ...
    with dl_col2:
        st.download_button(
            ...
            data=st.session_state.excel_report_bytes, # This uses the file we saved earlier
