# ==============================================================================
# FILE: agents/agent_5_reporter.py (DEFINITIVE, FINAL VERSION WITH STYLING AND BUG FIX)
# ==============================================================================
import pandas as pd
import io
import traceback
...
def report_finalizer_agent(aggregated_data, company_name):
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            # --- DEFINE RAINBOW PASTEL COLOR PALETTE (UNCHANGED) ---
            colors = {
            ...
            # --- 1. RENDER THE MAIN SHEETS (UNCHANGED) ---
            for sheet_name, template in [("Balance Sheet", MASTER_TEMPLATE["Balance Sheet"]), ("Profit and Loss", MASTER_TEMPLATE["Profit and Loss"])]:
                ...
                for row_data in template:
                    ...
                    cy_val, py_val = 0,0
                    if row_type in ["item", "item_sub", "item_no_alpha"]: note_total = aggregated_data.get(str(note), {}).get('total', {}); cy_val = note_total.get('CY', 0); py_val = note_total.get('PY', 0)
                    elif row_type == "total":
                    ...
                    if row_type in ["header", "sub_header"]: worksheet.write(row_num, 0, col_a, fmt_subheader); worksheet.write(row_num, 1, particulars, fmt_subheader)
                    elif row_type == "total": worksheet.write(row_num, 1, particulars, fmt_grand_total); worksheet.write_number(row_num, 3, cy_val, fmt_grand_total); worksheet.write_number(row_num, 4, py_val, fmt_grand_total)
                    elif cy_val is not None: worksheet.write(row_num, 0, col_a, current_format); worksheet.write(row_num, 1, particulars, current_format); worksheet.write_string(row_num, 2, str(note) if note else '', current_format); worksheet.write_number(row_num, 3, cy_val, current_format); worksheet.write_number(row_num, 4, py_val, current_format)
                    row_num += 1
            ...
            for note_num_str in sorted(NOTES_STRUCTURE_AND_MAPPING.keys(), key=lambda x: int(x.split('.')[0])):
                ...
                if 1 <= note_num_int <= 10: note_format = fmt_data_lia
                elif 11 <= note_num_int <= 20: note_format = fmt_data_asset
                else: note_format = fmt_data_eq
                ...
                # ******** THIS IS THE CORRECTED SECTION THAT FIXES THE CRASH ********
                worksheet.write(row_num, 0, "Total", fmt_total)
                worksheet.write_number(row_num, 1, note_data.get('total', {}).get('CY', 0), fmt_total) # Changed from total_format
                worksheet.write_number(row_num, 2, note_data.get('total', {}).get('PY', 0), fmt_total) # Changed from total_format
