# ==============================================================================
# FILE: agents/agent_5_reporter.py (CORRECTED WITH COLAB LOGIC)
# ==============================================================================
import pandas as pd
import io
import traceback
...
# --- Helper functions copied directly from the working Colab script ---
...
def create_main_sheet_df(aggregated_data, template):
    ...
    for row_template in template:
        ...
        if row_type in ["item", "item_no_alpha"]:
            note_str = str(note)
            if particulars == "Changes in inventories":
                opening_stock = aggregated_data.get('16', {}).get('total', {}).get('PY', 0)
                closing_stock = aggregated_data.get('16', {}).get('total', {}).get('CY', 0)
                row_data['As at March 31, 2025'] = closing_stock - opening_stock
            else:
                row_data['As at March 31, 2025'] = aggregated_data.get(note_str, {}).get('total', {}).get('CY', 0)
                row_data['As at March 31, 2024'] = aggregated_data.get(note_str, {}).get('total', {}).get('PY', 0)
        elif row_type == "total" and isinstance(note, list):
            cy_val, py_val = 0, 0
            for note_to_sum in note:
                note_str = str(note_to_sum)
                if note_str == '16' and "Expenses" in particulars:
                    opening_stock = aggregated_data.get('16', {}).get('total', {}).get('PY', 0)
                    closing_stock = aggregated_data.get('16', {}).get('total', {}).get('CY', 0)
                    cy_val -= (closing_stock - opening_stock)
                else:
            ...
            row_data['As at March 31, 2025'] = calc_totals.get('rev_cy', 0) - calc_totals.get('exp_cy', 0)
            row_data['As at March 31, 2024'] = calc_totals.get('rev_py', 0) - calc_totals.get('exp_py', 0)
            calc_totals['pbt_cy'], calc_totals['pbt_py'] = row_data['As at March 31, 2025'], row_data['As at March 31, 2024']
        elif note == "PAT":
            row_data['As at March 31, 2025'] = calc_totals.get('pbt_cy', 0) - calc_totals.get('tax_cy', 0)
            row_data['As at March 31, 2024'] = calc_totals.get('pbt_py', 0) - calc_totals.get('tax_py', 0)
...
def apply_main_sheet_styling(ws, template, company_name):
    ws.insert_rows(1, 3)
    ws.column_dimensions['B'].width = 65; ws.column_dimensions['C'].width = 8; ws.column_dimensions['D'].width = 20; ws.column_dimensions['E'].width = 20
    title_font = Font(name='Calibri', size=16, bold=True, color="0070C0"); subtitle_font = Font(name='Calibri', size=14, bold=True); header_font = Font(name='Calibri', size=11, bold=True); bold_font = Font(name='Calibri', size=11, bold=True)
    ...
    for cell in ws[4]: cell.font = header_font; cell.border = Border(bottom=Side(style='thin')); cell.alignment = Alignment(horizontal='center')
    for i, row_template in enumerate(template):
        ...
        for col_letter in ['D', 'E']:
            if isinstance(ws[f'{col_letter}{row_num}'].value, (int, float)): ws[f'{col_letter}{row_num}'].number_format = currency_format

def apply_note_sheet_styling(ws, note_title):
    ws.insert_rows(1); ws.column_dimensions['A'].width = 65; ws.column_dimensions['B'].width = 20; ws.column_dimensions['C'].width = 20
    title_font = Font(bold=True, size=14); header_font = Font(bold=True); total_font = Font(bold=True); sub_header_font = Font(bold=True)
    currency_format = '_(* #,##0.00_);_(* (#,##0.00);_(* "-"??_);_(@_)'
...
def report_finalizer_agent(aggregated_data, company_name):
    """AGENT 5: Constructs a detailed, styled report using the stable, two-stage
    process from the working Colab script."""
    print("\n--- Agent 5 (Report Finalizer): Building styled report... ---")
    try:
        output_buffer = io.BytesIO()
        # STAGE 1: Write all data using Pandas
        with pd.ExcelWriter(output_buffer, engine='openpyxl') as writer:
            ...
            for sheet_name, template in [("Balance Sheet", MASTER_TEMPLATE["Balance Sheet"]), ("Profit and Loss", MASTER_TEMPLATE["Profit and Loss"])]:
                df = create_main_sheet_df(aggregated_data, template)
        ...
        # STAGE 2: Re-open with openpyxl to apply styling
