...
def report_finalizer_agent(aggregated_data, company_name):
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            fmt_header = workbook.add_format({'bold': True, 'bg_color': colors['pink'], 'font_color': colors['white'], 'border': 1, 'align': 'center', 'valign': 'vcenter'})
            fmt_subheader = workbook.add_format({'bold': True, 'bg_color': colors['light_pink'], 'font_color': colors['dark_grey']})
            ...
            for sheet_name, template in [("Balance Sheet", MASTER_TEMPLATE["Balance Sheet"]), ("Profit and Loss", MASTER_TEMPLATE["Profit and Loss"])]:
                ...
                for row_data in template:
                    ...
                    if row_type in ["item", "item_sub", "item_no_alpha"]:
                        ...
                        cy_val = note_total.get('CY', 0); py_val = note_total.get('PY', 0)
                    elif row_type == "total":
                        cy_val = get_total(note, 'CY'); py_val = get_total(note, 'PY')

                    # Write row data
                    if row_type in ["header", "sub_header"]:
                        worksheet.write(row_num, 0, col_a); worksheet.write(row_num, 1, particulars, fmt_subheader)
                    elif row_type == "total":
                        worksheet.write(row_num, 1, particulars, fmt_grand_total)
                        worksheet.write_number(row_num, 3, cy_val, fmt_grand_total)
                        worksheet.write_number(row_num, 4, py_val, fmt_grand_total)
                    else:
                        worksheet.write(row_num, 0, col_a)
                        worksheet.write(row_num, 1, particulars)
                        worksheet.write_string(row_num, 2, str(note) if note else '')
                        worksheet.write_number(row_num, 3, cy_val, num_format)
                        worksheet.write_number(row_num, 4, py_val, num_format)
                    row_num += 1
            ...
            for note_num_str in sorted(NOTES_STRUCTURE_AND_MAPPING.keys(), key=lambda x: int(x.split('.')[0])):
                ...
                def write_note_level(items, indent_level=0):
                    ...
                    for key, value in items.items():
                        ...
                        if isinstance(value, dict) and 'CY' in value:
                            worksheet.write(row_num, 0, f"{prefix}{key}")
                ...
                worksheet.write_number(row_num, 1, note_data.get('total', {}).get('CY', 0), total_format)
                worksheet.write_number(row_num, 2, note_data.get('total', {}).get('PY', 0), total_format)
