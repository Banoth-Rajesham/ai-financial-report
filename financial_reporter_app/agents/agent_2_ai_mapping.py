...
def ai_mapping_agent(source_particulars, mapping_structure, api_url=None, api_key=None):
    ...
    This agent is now decoupled from Streamlit and accepts API credentials as arguments.
    """
    print("\n--- Agent 2 (AI Mapping): Searching for unrecognized terms... ---")

    updated_mapping = copy.deepcopy(mapping_structure)

    # Helper function to recursively find all known keywords
    ...
    # Proceed with API call only if credentials are provided
    ...
    try:
        ...
        return updated_mapping # Return original mapping on failure

    # Helper function to find where to add the new keyword
    def find_and_update(node, target_title, new_keyword):
        for note_data in node.values():
            if note_data.get('title') == target_title:
                # Find the first list of keywords to append to (a reasonable default)
